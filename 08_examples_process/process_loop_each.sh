#!/bin/bash
# @author: hoojo
# @email: hoojo_@126.com
# @github: https://github.com/hooj0
# @create date: 2018-05-29 22:44:12
# @copyright by hoojo@2018
# @changelog Added bash `process -> loop each` example



# ==========================================================================
#       for / while ———— 循环
# ==========================================================================
# for 和 while 循环
# --------------------------------------------------------------------------


# ==========================================================================
# 示例：for循环遍历输出数组
# ==========================================================================
my_arry=(a b "c","d" abc)
echo "-------FOR循环遍历输出数组--------"
for i in ${my_arry[@]};
do
  echo $i
done


# ==========================================================================
# 示例：while循环输出 使用 let i++ 自增
# ==========================================================================
echo "-------::::WHILE循环输出 使用 let i++ 自增:::::---------"
j=0
while [ $j -lt ${#my_arry[@]} ]
do
  echo ${my_arry[$j]}
  let j++
done


# ==========================================================================
# 示例：while循环输出 使用 let  "n++" 自增: 多了双引号，其实不用也可以
# ==========================================================================
echo "--------:::WHILE循环输出 使用 let  "n++" 自增: 多了双引号，其实不用也可以:::---------"
n=0
while [ $n -lt ${#my_arry[@]} ]
do
  echo ${my_arry[$n]}
  let "n++"
done


# ==========================================================================
# 示例：while循环输出 使用 let m+=1 自增，这种写法其他编程中也常用
# ==========================================================================
echo "---------::::WHILE循环输出 使用 let m+=1 自增，这种写法其他编程中也常用::::----------"
m=0
while [ $m -lt ${#my_arry[@]} ]
do
  echo ${my_arry[$m]}
  let m+=1
done


# ==========================================================================
# 示例：while循环输出 使用 a=$[$a+1] 自增，这种写法比较麻烦
# ==========================================================================
echo "-------::WHILE循环输出 使用 a=$[$a+1] 自增，这种写法比较麻烦::----------"
a=0
while [ $a -lt ${#my_arry[@]} ]
do
 echo ${my_arry[$a]}
 a=$[$a+1]
done

read status